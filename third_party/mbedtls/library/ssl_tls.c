...
#if defined(ESP8266_PLATFORM)
#define MBEDTLS_SSL_OUTBUFFER_LEN  ( 2920               \
...
pmbedtls_finished	pfinished = NULL;

static pmbedtls_finished mbedtls_ssl_finished_new(int len)
{
    pmbedtls_finished finished = (pmbedtls_finished)zalloc(sizeof(mbedtls_finished));
    if (finished){
        finished->finished_len = len;
        finished->finished_buf = (uint8*)zalloc(finished->finished_len + 1);
        if (finished->finished_buf == NULL){
            free(finished);
            finished = NULL;
        }
    }
...
static void mbedtls_ssl_finished_free(pmbedtls_finished *pfinished)
{
    free((*pfinished)->finished_buf);
    free(*pfinished);
    *pfinished = NULL;
}

static int mbedtls_ssl_finished_handle(mbedtls_ssl_context *ssl)
{   
    int ret = 0;	
    const size_t len = MBEDTLS_SSL_OUTBUFFER_LEN;

	ssl->out_buf = (unsigned char*)zalloc(len);
	if (ssl->out_buf != NULL){	   
	    ssl->out_ctr = ssl->out_buf;
	    ssl->out_hdr = ssl->out_buf +  8;
	    ssl->out_len = ssl->out_buf + 11;
	    ...
	    memcpy(ssl->out_ctr, pfinished->finished_buf, pfinished->finished_len);
	    mbedtls_ssl_finished_free(&pfinished);
	} else{
...
int mbedtls_ssl_finished_write(mbedtls_ssl_context *ssl)
{
    int ret = 0;
    
    pfinished = mbedtls_ssl_finished_new(ssl->out_msglen + 29);
    if (pfinished != NULL)
    	memcpy(pfinished->finished_buf, ssl->out_ctr, pfinished->finished_len);
	else
		ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
    return ret;
}
...
int mbedtls_ssl_finished_hanshake(mbedtls_ssl_context *ssl)
{
	...
	mbedtls_ssl_finished_handle(ssl);
}
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
	...
	if( ssl->out_buf != NULL )
	{
		mbedtls_zeroize( ssl->out_buf, MBEDTLS_SSL_OUTBUFFER_LEN );
		mbedtls_free( ssl->out_buf );
	}
#endif
